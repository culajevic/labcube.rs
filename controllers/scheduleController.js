...
///////////////////
  ...
  for (let i = 0; i < req.body[1].analysis.length; i++) {
    analysisArr.push({
      analysis: req.body[1].analysis[i].name,
  ...
  let getBullets = analysisForEmail.map((item) => `<li>${item}</li>`);
                    ...
                    <table align="left" border="0" cellpadding="0" cellspacing="0" style="max-width:100%; min-width:100%;" width="100%" class="mcnTextContentContainer">
                        <tbody><tr>
                            ...
                            <td valign="top" class="mcnTextContent" style="padding-top:0; padding-right:18px; padding-bottom:9px; padding-left:18px;">
                                ...
                                <a href="https://labcube.rs/politika-privatnosti" target="_blank">politika privatnosti</a>&nbsp;
                                | <a href="https://labcube.rs/uslovi-koriscenja" target="_blank">uslovi korišćenja</a>&nbsp;
                                | <a href="https://labcube.rs/uslovi-placanja" target="_blank">uslovi plaćanja</a>&nbsp;
                                | <a href="https://labcube.rs" target="_blank">labcube.rs</a>
                ...
                <table align="left" border="0" cellpadding="0" cellspacing="0" style="max-width:100%; min-width:100%;" width="100%" class="mcnTextContentContainer">
                    <tbody><tr>
                        ...
                        <td valign="top" class="mcnTextContent" style="padding: 0px 18px 9px; line-height: 200%;">
                            ...
                            <h2 style="text-align: center;"><style="font-size:20px;">Laboratorija <span style="color:#1D88E5;"><a href="${req.get(
                              "referer"
                            )}" style="text-decoration:none;">${
      getLabData[0].labName
    }</a></span> te očekuje!</h2>
&nbsp;

<p style="line-height: 200%;"><span style="font-size:18px"><img data-file-id="6005129" height="18" src="https://mcusercontent.com/a10f76f8952b08c139ac9f0ef/images/e8d426a8-ac5d-fdfd-90c0-f01d6f11be9a.png" style="border: 0px  ; width: 17px; height: 18px; margin: 0px; margin-bottom:-3px;" width="17"> ${
      getLabData[0].address
    }, ${getLabData[0].placeId.place}<br>
<img data-file-id="6005133" height="18" src="https://mcusercontent.com/a10f76f8952b08c139ac9f0ef/images/c8f09b5b-8a6c-2f04-65ca-c09acabd4a6a.png" style="border: 0px  ; width: 10px; height: 18px; margin: 0px; margin-left:1px; margin-bottom:-3px; margin-right:5px;" width="10"> ${
      getLabData[0].phone
    }</span></p>
                    ...
                    <tbody><tr>
                        ...
                        <td valign="top" class="mcnTextContent" style="padding-top:0; padding-right:18px; padding-bottom:9px; padding-left:18px;">
							...
							<table>
								...
								<tr>
								<td style="text-align:center;">${getLabData[0].workingHours.monday.opens}  ${
      getLabData[0].workingHours.monday.closes
    }</td>
								<td style="text-align:center;">${getLabData[0].workingHours.tuesday.opens}  ${
      getLabData[0].workingHours.tuesday.closes
    }</td>
								<td style="text-align:center;">${getLabData[0].workingHours.wednesday.opens}  ${
      getLabData[0].workingHours.wednesday.closes
    }</td>
								<td style="text-align:center;">${getLabData[0].workingHours.thursday.opens}  ${
      getLabData[0].workingHours.thursday.closes
    }</td>
								<td style="text-align:center;">${getLabData[0].workingHours.friday.opens}  ${
      getLabData[0].workingHours.friday.closes
    }</td>
								<td style="text-align:center;">${getLabData[0].workingHours.saturday.opens}  ${
      getLabData[0].workingHours.saturday.closes
    }</td>
								<td style="text-align:center;">${getLabData[0].workingHours.sunday.opens}  ${
      getLabData[0].workingHours.sunday.closes
    }</td>
								</tr>  
								</table>
                        </td>
...
exports.resultsInterpretation = [
  authCheck,
  async (req, res) => {
    const countTotal = await Schedule.countDocuments({
    ...
    const resultsForInterpretation = await Schedule.find({
      ...
      .populate("owner")
      .sort({ createdDate: -1 });
    res.render("resultsInterpretation", {
      resultsForInterpretation,
      title: "Tumačenje rezultata",
      page,
      countTotal,
      pages,
      paginationURL: "resultsInterpretation",
    });
  },
];

exports.otherResultsInterpretation = [
  authCheck,
  async (req, res) => {
    ...
    const otherResultsForInterpretation = await Result.find({})
      ...
      .populate("owner")
      .sort({ submitedDate: -1 });
    res.render("otherResultsInterpretation", {
      otherResultsForInterpretation,
      title: "Tumačenje ostalih rezultata",
      page,
      countTotal,
      pages,
      paginationURL: "otherResultsInterpretation",
    });
  },
];
  ...
  (req, res) => {
    upload(req, res, (err) => {
      if (err) {
        req.flash(
          "error_msg",
          "Dozvoljeni formati fajlova su pdf, jepg, jpg, png i veličina fajla mora biti manja od 3MB"
        );
...
exports.otherResultsInterpretationValues = [
  authCheck,
  async (req, res) => {
    const groupNames = await Group.find({}, { name: 1, slug: 1, _id: 0 }).sort({
      name: 1,
    });
    const findOtherResult = await Result.find({ _id: req.params.id }).populate(
      "userId"
    );
    ...
    res.render("interpretatedOtherResults", {
      findOtherResult: findOtherResult,
      user: req.user,
      groupNames,
      findPreviousResults,
    });
...
exports.resultsInterpretationValues = [
  authCheck,
  async (req, res) => {
    const groupNames = await Group.find({}, { name: 1, slug: 1, _id: 0 }).sort({
      name: 1,
    ...
    const analysisValues = await Schedule.find({ _id: req.params.id })
      .populate("lab")
      .populate("user");
    res.render("analysisInterpretation", {
      analysisValues,
      user: req.user,
      groupNames,
    });
...
exports.lockTheOtherInterpretation = async (req, res) => {
  // console.log(req.body[0].ownerId)
  // console.log(req.body[0].interpretationId)
  let check = await Result.find({
    $and: [{ _id: req.body[0].interpretationId }, { owner: { $exists: true } }],
  }).countDocuments();
  console.log(check);
  if (check == 0) {
...
exports.myResultLabCube = [
  ...
  async (req, res) => {
    ...
    const myResults = await Result.findOne({
      $and: [{ _id: req.params.id }, { status: "Završeno" }],
    })
    ...
    res.render("myResultsLabCube", {
      myResults,
      groupNames,
      feedbackSent,
      user: req.user,
      title: "LabCube | Moji rezultati",
    });
  },
];
