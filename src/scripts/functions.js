...
exports.renderAnalysisResult = (analysis, prices, resultDiv, itemsArray, i) => {
  ...
  if (prices[i].availableHC[0] == true) {
    ...
    hospitalIcon.setAttribute(
      "title",
      "Analizu je moguće uraditi u domu zdravlja o trošku zdravstvenog osiguranja."
    );
  } else {
    ...
    hospitalIcon.setAttribute(
      "title",
      "Analizu nije moguće uraditi u domu zdravlja o trošku zdravstvenog osiguranja."
    );
  }
  ...
  if (analysisPositionArr === -1) {
    ...
    addAnalysisBtn.className =
      "btn btn-outline-success float-right btn-block text-uppercase deleteAnalysis";
...
exports.displayBasket = (itemsArray) => {
  ...
  itemsArray.forEach((analysis) => {
    ...
    selectedAnalysis.insertBefore(
      analysisAdded,
      selectedAnalysis.childNodes[analysisPositionArr]
    );
  });
};
    ...
    if (e.target.classList.contains("remove-analysis-from-basket")) {
      ...
      if (itemsArray.length == 0) {
        ...
        let resultSection = document.getElementById("resultsLabDetails")
          ? document.getElementById("resultsLabDetails")
          : "";
        if (resultSection) {
...
exports.addAnalysis = (itemsArray, resultDiv, checkout) => {
  ...
  resultDiv.addEventListener("click", (e) => {
    if (
      ...
      e.target.classList.contains("addAnalysis") &&
      itemsArray.length < 30
    ) {
      ...
      logoImg.setAttribute(
        "src",
        `/images/${e.target.getAttribute("data-groupimg")}`
      );
      ...
      let groupImage = document.createElement("img");
      groupImage.classList = "labGroupIconSelectedAnalysis";
      groupImage.setAttribute(
        "src",
        "/images/" + e.target.getAttribute("data-groupImg")
      );
      // console.log(e.target.getAttribute('data-analysisName'))
      let newAnalysisName = e.target
        .getAttribute("data-analysisName")
        .replace(" - ", " ")
        .replace("/", " ");
      //creating text with analysis name
      let analysisName = document.createTextNode(
        e.target.getAttribute("data-analysisName")
      );
      let analysisLink = document.createElement("a");
      let slug = newAnalysisName.split(" ");
      let urlSlug = slug.join("-");
      ...
      if (analysisPositionArr !== -1) {
        ...
        e.target.className =
          "btn btn-danger btn-block text-uppercase deleteAnalysis";
    ...
    if (searchString.value.length >= 2) {
      setTimeout(function () {
        ...
        window.location.href =
          "/results/?name=" + searchString + "&filter=" + filterValue;
      }, 700);
    }
  });
};
  ...
  fetch("/lab/" + searchStr).then((data) => {
    data.json().then((result) => {
      ...
      for (i = 0; i < result.length; i++) {
        ...
        labTemplate.innerHTML += `
          <div class="lab-card roundedBox boxshadowBckg">
            <div class="testingFlex">
                  ...
                  <div>
                  <img src="" class="labInfoWindowVerified partnerIcon${i}" title="LabCube partner">
                   ...
                   <div class="labInfoWindow">
                       ...
                       <p class="labInfoWindowGrad">${
                         result[i].placeId.place
                       } </p>
                       <p class="labInfoWindowTelefoni">${result[i].phone.join(
                         ", "
                       )}</p>
                   ...
                   <div class="labInfoFooter">
                       ...
                       <div class="labInfoRadnoVremeDetalji">
                         <p class="daysInWeek monday${i} text-center">P<span>${
          result[i].workingHours.monday.opens
        } - ${result[i].workingHours.monday.closes}</span></p>
                         <p class="daysInWeek tuesday${i} text-center">U<span>${
          result[i].workingHours.tuesday.opens
        } - ${result[i].workingHours.tuesday.closes}</span></p>
                         <p class="daysInWeek wednesday${i} text-center">S<span>${
          result[i].workingHours.wednesday.opens
        } - ${result[i].workingHours.wednesday.closes}</span></p>
                         <p class="daysInWeek thursday${i} text-center">Č<span>${
          result[i].workingHours.thursday.opens
        } - ${result[i].workingHours.thursday.closes}</span></p>
                         <p class="daysInWeek friday${i} text-center">P<span>${
          result[i].workingHours.friday.opens
        } - ${result[i].workingHours.friday.closes}</span></p>
                         <p class="daysInWeek saturday${i} text-center">S<span>${
          result[i].workingHours.saturday.opens
        } - ${result[i].workingHours.saturday.closes}</span></p>
                         <p class="daysInWeek sunday${i} text-center">N<span>${
          result[i].workingHours.sunday.opens
        } - ${result[i].workingHours.sunday.closes}</span></p>
                       </div>
                    </div>
                    <button type="button" class="btn btn-block btnLabDetails buttonId mt-2" data-labName="laboratorija/${
                      result[i].slug
                    }">saznaj više</button>
        ...
        let privateInsurance = document.querySelector(
          ".privateInssuranceIcon" + i
        );
        ...
        let partnerIcon = document.querySelector(".partnerIcon" + i);
        ...
        labDetailsBtn.forEach((item) => {
          item.addEventListener("click", (e) => {
            ...
            window.location = `/${e.target.getAttribute(
              "data-labName"
            )}/${passIds}`;
          });
        });
        ...
        if (result[i].antigen) {
          antigen.setAttribute(
            "src",
            "/images/covidIcon.svg-1615411728326.svg"
          );
        } else {
          antigen.remove();
        }
        ...
        if (result[i].open24h) {
          ...
          if (closingIn < 60 && closingIn > 0) {
            ...
            nowTimeStamp > todayOpenTime.getTime() &&
      ...
      if (result.getPrices.length > 0) {
        ...
        for (let i = 0; i < result.getPrices.length; i++) {
          if (day == currentDayNum) {
            let openTime =
              result.getPrices[i].lab[0].workingHours[currentDay].opens;
            let closingTime =
              result.getPrices[i].lab[0].workingHours[currentDay].closes;
            let todayOpenTime = new Date(today + " " + openTime + ":00");
            let todayClosingTime = new Date(today + " " + closingTime + ":00");
            let nowTimeStamp = now.getTime();
            let closingSoon = todayClosingTime - nowTimeStamp;
            ...
            if (result.getPrices[i].lab[0].open24h) {
              status = "open";
              labStatus.push({
                id: result.getPrices[i].lab[0]._id,
                status: status,
              });
        ...
        <div class="lab-card roundedBox">
          <div>
          ${
            result.getPrices[i].lab[0].partner
              ? '<img src=/images/labPartner.svg class="labInfoWindowOsiguranje partnerIcon${i}" title="LabCube partner">'
              : ""
          }
          ${
            result.getPrices[i].lab[0].private
              ? '<img src=/images/osiguranje.svg class="labInfoWindowOsiguranje privateInssuranceIcon${i}" title="laboratorija sarađuje sa privatnim osiguranjem">'
              : ""
          }
          ${
            result.getPrices[i].lab[0].disability
              ? '<img src=/images/disability.svg class="labInfoWindowVerified accreditedIcon${i}" title="ulaz prilagođen invalidima">'
              : ""
          }
          ${
            result.getPrices[i].lab[0].patronage
              ? '<img src=/images/patronaza2.svg class="labInfoWindowVerified accreditedIcon${i}" title="dostupna patronaža">'
              : ""
          }
          ${
            result.getPrices[i].lab[0].accredited
              ? '<img src=/images/verified.svg class="labInfoWindowVerified accreditedIcon${i}" title="laboratorija je akreditovana">'
              : ""
          }
          ${
            result.getPrices[i].lab[0].antigen
              ? '<img src=/images/covidIcon.svg-1615411728326.svg class="labInfoWindowVerified labAntigenIcon accreditedIcon${i}" title="radi antigenski test">'
              : ""
          }
          <span class="labInfoWindowTitle">${
           ...
           <div class="labInfoWindow">
               ...
               <p class="labInfoWindowAdresa">${
                 result.getPrices[i].lab[0].address
               }</p>
               <p class="labInfoWindowGrad">${
                 result.getPrices[i].labPlace[0].place
               }</p>
               <p class="labInfoWindowTelefoni"> ${
                 result.getPrices[i].lab[0].phone
               } </p>
           </div>
           <div class="labInfoFooter">
               <img src="/images/radnoVreme_black.svg" class="labInfoWindowWorkingHoursIcon">
               ...
               <div class="labInfoRadnoVremeDetalji">
                 <p class="daysInWeek monday${result[i]} text-center ${
            day == 1 ? labStatus[i].status : ""
          }">P<span>${result.getPrices[i].lab[0].workingHours.monday.opens} - ${
            result.getPrices[i].lab[0].workingHours.monday.closes
          }</span></p>
                 <p class="daysInWeek tuesday${result[i]} text-center ${
            day == 2 ? labStatus[i].status : ""
          }">U<span>${
            result.getPrices[i].lab[0].workingHours.tuesday.opens
          } - ${
            result.getPrices[i].lab[0].workingHours.tuesday.closes
          }</span></p>
                 <p class="daysInWeek wednesday${result[i]} text-center ${
            day == 3 ? labStatus[i].status : ""
          }">S<span>${
            result.getPrices[i].lab[0].workingHours.wednesday.opens
          } - ${
            result.getPrices[i].lab[0].workingHours.wednesday.closes
          }</span></p>
                 <p class="daysInWeek thursday${result[i]} text-center ${
            day == 4 ? labStatus[i].status : ""
          }">Č<span>${
            result.getPrices[i].lab[0].workingHours.thursday.opens
          } - ${
            result.getPrices[i].lab[0].workingHours.thursday.closes
          }</span></p>
                 <p class="daysInWeek friday${result[i]} text-center ${
            day == 5 ? labStatus[i].status : ""
          }">P<span></span>${
            result.getPrices[i].lab[0].workingHours.friday.opens
          } - ${result.getPrices[i].lab[0].workingHours.friday.closes}</p>
                 <p class="daysInWeek saturday${result[i]} text-center ${
            day == 6 ? labStatus[i].status : ""
          }">S<span></span>${
            result.getPrices[i].lab[0].workingHours.saturday.opens
          } - ${result.getPrices[i].lab[0].workingHours.saturday.closes}</p>
                 <p class="daysInWeek sunday${result[i]} text-center ${
            day == 0 ? labStatus[i].status : ""
          }">N<span></span>${
            result.getPrices[i].lab[0].workingHours.sunday.opens
          } - ${result.getPrices[i].lab[0].workingHours.sunday.closes}</p>
               </div>
            </div>
            <a class="btn btn-block btnLabDetails buttonId mt-2" href="laboratorija/${
              result.getPrices[i].lab[0].slug
            }/${passIds}">saznaj više</a>
         </div>`;
        ...
        let map = new google.maps.Map(
          document.getElementById("mapPrices"),
          options
        );
        map.setCenter({
        ...
        function addMarker(
          ...
          let content = `<div class="" style="min-height:142px; max-width:380px;">
                        ...
                        <p class="">${city}</p>
                        <p class="labInfoWindowTelefoni">${phonesForClick.join(
                          ", "
                        )}</p>
                        ...
                        <div class="radnoVremeDetalji">
                          <p class="whInside text-center ${
                            day == 1 ? labStatus[i].status : ""
                          }">P<span>${workinghours.monday.opens} - ${
            workinghours.monday.closes
          }</span></p>
                          <p class="whInside text-center ${
                            day == 2 ? labStatus[i].status : ""
                          }">U<span>${workinghours.tuesday.opens} - ${
            workinghours.tuesday.closes
          }</span></p>
                          <p class="whInside text-center ${
                            day == 3 ? labStatus[i].status : ""
                          }">S<span>${workinghours.wednesday.opens} - ${
            workinghours.wednesday.closes
          }</span></p>
                          <p class="whInside text-center ${
                            day == 4 ? labStatus[i].status : ""
                          }">Č<span>${workinghours.thursday.opens} - ${
            workinghours.thursday.closes
          }</span></p>
                          <p class="whInside text-center ${
                            day == 5 ? labStatus[i].status : ""
                          }">P<span>${workinghours.friday.opens} - ${
            workinghours.friday.closes
          }</span></p>
                          <p class="whInside text-center ${
                            day == 6 ? labStatus[i].status : ""
                          }">S<span>${workinghours.saturday.opens} - ${
            workinghours.saturday.closes
          }</span></p>
                          <p class="whInside text-center ${
                            day == 0 ? labStatus[i].status : ""
                          }">N<span>${workinghours.sunday.opens} - ${
            workinghours.sunday.closes
          }</span></p>
                        </div>
                      </div>`;
